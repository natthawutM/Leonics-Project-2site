...
$link1 = connectToDB1();
	$strSQL = "SELECT DatetimeLocal,BDI1_BDI_Total_Power_kW,BDI2_BDI_Total_Power_kW FROM graph where  year(DatetimeLocal)= $y_1 and month(DatetimeLocal)= $m_1 and day(DatetimeLocal)= $d_1 order by DatetimeLocal ASC";
			...
			while($objResult = mysql_fetch_array($objQuery))
			{
				...
				if($objResult['BDI1_BDI_Total_Power_kW']==-0.999||($objResult['BDI1_BDI_Total_Power_kW']==NULL)){$Input_V1=0;}else{$Input_V1=$objResult['BDI1_BDI_Total_Power_kW'];}
				if($objResult['BDI2_BDI_Total_Power_kW']==-0.999||($objResult['BDI2_BDI_Total_Power_kW']==NULL)){$Input_V2=0;}else{$Input_V2=$objResult['BDI2_BDI_Total_Power_kW'];}
				$sum=$Input_V1+$Input_V2;
				$nn_Pv_kW1_1.="[Date.UTC(".$Year1[$i].",". $Month1[$i].",". $day1[$i].",". $H1[$i].",". $M1[$i].",". $S1[$i]."),".$sum."],";
				$i++;
...
//Highcharts.chart('container', {
    ...
    tooltip: {
        ...
        pointFormat: '<span style="font-family:Courier; color:transparent; font-size: 7px;">&nbsp;&nbsp;</span>{point.y:,.2f} kW  ',
        shared: false,
	style: {
			fontFamily: 'Calibri',
			color: '#000',
...
//alert("vvv");
		xmlDoc=loadXMLDoc("../xml/Xml_bdi.php");
		var Y=new Number();
