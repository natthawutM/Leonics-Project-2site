...
$link1 = connectToDB1();
	$strSQL = "SELECT DatetimeLocal,Irradiance_W_m2 FROM graph where year(DatetimeLocal)= $y_1 and month(DatetimeLocal)= $m_1 and day(DatetimeLocal)= $d_1 order by DatetimeLocal ASC";
			...
			while($objResult = mysql_fetch_array($objQuery))
			{
				...
				if($objResult['Irradiance_W_m2']==-0.999||($objResult['Irradiance_W_m2']==NULL)){$Input_V1=0;}else{$Input_V1=$objResult['Irradiance_W_m2'];}
...
//Highcharts.chart('container', {
    ...
    tooltip: {
        ...
        pointFormat: '<span style="font-family:Courier; color:transparent; font-size: 8.5px;">&nbsp;&nbsp;</span>{point.y:,.1f} W/m<sup>2</sup>',
        shared: false,
	style: {
			fontFamily: 'Calibri',
			color: '#000',
...
//alert("vvv");
		xmlDoc=loadXMLDoc("../xml/Xml_irr.php");
		var Y=new Number();
