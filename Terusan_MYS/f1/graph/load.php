...
$link1 = connectToDB1();
	$strSQL = "SELECT DatetimeLocal,Load_PM_Total_P_kW FROM graph where  year(DatetimeLocal)= $y_1 and month(DatetimeLocal)= $m_1 and day(DatetimeLocal)= $d_1 order by DatetimeLocal ASC";
			...
			while($objResult = mysql_fetch_array($objQuery))
			{
				...
				if($objResult['Load_PM_Total_P_kW']==-0.999||($objResult['Load_PM_Total_P_kW']==NULL)){$Input_V1=0;}else{$Input_V1=$objResult['Load_PM_Total_P_kW'];}
...
//Highcharts.chart('container', {
    ...
    tooltip: {
        ...
        pointFormat:'<b><span style="font-family:Courier; color:transparent; font-size: 10px;">&nbsp;</span>{point.y:,.2f} kW</b>  ',
        shared: true,
	style: {
			fontFamily: 'Calibri',
			color: '#000',
...
//alert("vvv");
		xmlDoc=loadXMLDoc("../xml/Xml_load.php");
		var Y=new Number();
