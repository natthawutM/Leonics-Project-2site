   ...
   (function(symbolName) {
      ...
      Symbol.bindTriggerAction(compId, symbolName, "Default Timeline", 0, function(sym, e) {
         ...
         dataType: "xml",
			 ...
			 sym.$("Load_kW").html(Sum_Load.toFixed(2));
         ...
         success: function(xml) {
			...
			sym.$("BattUp").hide();
		//------------------
		 	 sym.$("Batt1_v").html(($(xml).find('Batt_Avg_Voltage').text()));
			 sym.$("Batt1_I").html(($(xml).find('Batt_Sum_Current').text()));
         	 sym.$("Batt1_soc").html(($(xml).find('Batt_Avg_SOC').text()));
		
		//------------------- Check Batt
         	    Batt_I1 = $(xml).find('Batt_Sum_Current').text();
