...
$sum=0;
	...
	$strSQL1 = "SELECT DatetimeLocal,BDI1_BDI_Total_Power_kW,BDI2_BDI_Total_Power_kW FROM graph where  year(DatetimeLocal)= $Y and month(DatetimeLocal)= $m and day(DatetimeLocal)= $d order by DatetimeLocal  DESC LIMIT 1 ";
	//print $strSQL1."<br>";
			...
			while($objResult = mysql_fetch_array($objQuery1))
			{
				...
				if(($objResult['BDI1_BDI_Total_Power_kW']==-0.999)||($objResult['BDI1_BDI_Total_Power_kW']==NULL)){$PV=0;}else{$PV=$objResult['BDI1_BDI_Total_Power_kW'];}
				if(($objResult['BDI2_BDI_Total_Power_kW']==-0.999)||($objResult['BDI2_BDI_Total_Power_kW']==NULL)){$PV1=0;}else{$PV1=$objResult['BDI2_BDI_Total_Power_kW'];}
				$sum=$PV+$PV1;
			}
