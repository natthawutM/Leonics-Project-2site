...
$sum=0;
	...
	$strSQL1 = "SELECT DatetimeLocal,Irradiance_W_m2 FROM graph where  year(DatetimeLocal)= $Y and month(DatetimeLocal)= $m and day(DatetimeLocal)= $d  order by DatetimeLocal  DESC LIMIT 1 ";
	//print $strSQL1."<br>";
			...
			while($objResult = mysql_fetch_array($objQuery1))
			{
				...
				if(($objResult['Irradiance_W_m2']==-0.999)||($objResult['Irradiance_W_m2']==NULL)){$PV1=0;}else{$PV1=$objResult['Irradiance_W_m2'];}
			}
