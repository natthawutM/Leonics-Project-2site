...
$sum=0;
	...
	$strSQL1 = "SELECT DatetimeLocal,GCI_Sum_AC_kW FROM graph where  year(DatetimeLocal)= $Y and month(DatetimeLocal)= $m and day(DatetimeLocal)= $d    order by DatetimeLocal  DESC LIMIT 1 ";
	//print $strSQL1."<br>";
			...
			while($objResult = mysql_fetch_array($objQuery1))
			{
				...
				if(($objResult['GCI_Sum_AC_kW']==-0.999)||($objResult['GCI_Sum_AC_kW']==NULL)){$PV1=0;}else{$PV1=$objResult['GCI_Sum_AC_kW'];}
			}
