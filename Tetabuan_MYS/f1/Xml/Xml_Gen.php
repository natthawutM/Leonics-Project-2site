...
$sum=0;
	...
	$strSQL1 = "SELECT DatetimeLocal,Gen_kW FROM graph where  year(DatetimeLocal)= $Y and month(DatetimeLocal)= $m and day(DatetimeLocal)= $d    order by DatetimeLocal  DESC LIMIT 1 ";
	//print $strSQL1."<br>";
			...
			while($objResult = mysql_fetch_array($objQuery1))
			{
				...
				if(($objResult['Gen_kW']==-0.999)||($objResult['Gen_kW']==NULL)){$PV1=0;}else{$PV1=$objResult['Gen_kW'];}
