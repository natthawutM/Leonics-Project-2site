...
mysql_select_db ("dwr_rbr")or die ("can not connect to your database");
	...
	$strSQL = "SELECT DatetimeLocal,PV_kWh,ACin_kWh,LoadPM_Import_kWh,WindPM_Import_kWh,BDI_AC_Feed2Grid_kWh,Irrt_kWh_m2 FROM energylog where year(DatetimeLocal)='$y_1'  order by DatetimeLocal asc";
...
<title>Yearly Energy - Department of Water Resource</title>
...
<div class="wrap">
  <div class="card">
    <div class="card-head">
      <div class="card-title"><span class="dot"></span>Yearly Energy Report - <b><?php echo $date_name;?></b></div>
      <form id="form1" name="form1" method="POST" action="yearly.php" class="date-row">
        <select name="y" id="y">
          <option value="<?php print $y_1; ?>"><?php print $y_1; ?></option>
          <?php for($i=2026;$i<=date('Y');$i++){?><option value="<?php print $i;?>"><?php print $i;?></option><?php }?>
        </select>
...
var _cat=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
...
$(document).ready(function() {
  Highcharts.chart('container', {
    ...
    yAxis:[
      ...
      /*,{ min:0, max:150, tickInterval:50, startOnTick:false, endOnTick:false, opposite:true,
