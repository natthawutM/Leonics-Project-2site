...
$dx = $y1.$m1.$d1."000000";
$dy = $y2.$m2.$d2."235959";
header('Content-disposition: attachment; filename=dwr_rbr.csv');
...
// delete outfile if exist
    if (file_exists("c:/wamp/www/dwr_rbr/f1/Download/dwr_rbr.csv"))
    unlink("c:/wamp/www/dwr_rbr/f1/Download/dwr_rbr.csv");
...
" from main_server_table where Datetimeserverhccu >= $dx and Datetimeserverhccu <= $dy order by Datetimeserverhccu asc into outfile 'c:/wamp/www/dwr_rbr/f1/Download/dwr_rbr.csv' fields terminated by ' ' LINES TERMINATED BY '\r\n'";
...
"GTP2_Power3_kW,Aload_Voltage_L1,Aload_Voltage_L2,Aload_Voltage_L3,Aload_Current_I1,Aload_Current_I2,Aload_Current_I3,".
"Aload_Power1_kW,Aload_Power2_kW,Aload_Power3_kW".
" from main_server_table order by Datetimeserverhccu desc limit $n_rec into outfile 'c:/wamp/www/dwr_rbr/f1/Download/dwr_rbr.csv' fields terminated by ' ' LINES TERMINATED BY '\r\n'";
    $result = mysql_query($strQuery) or die(mysql_error());
    mysql_close($link);
   }

   if (filesize("c:/wamp/www/dwr_rbr/f1/Download/dwr_rbr.csv") == 0)
   {
    echo "No data found";
   }
   else
   {
    readfile("c:/wamp/www/dwr_rbr/f1/Download/dwr_rbr.csv");
   }
